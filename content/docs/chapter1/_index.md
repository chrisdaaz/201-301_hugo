...
# Page metadata.
title: Bonding to Crystal Structure
...
where $k$ is the spring constant - a measure of how stiff the spring is. A higher $k$ means a larger restoring force when the spring is displaced. Fig. \@ref(fig:SpringSystem) displays a red (solid) line that follows Hooke's law (Eq. \@ref(eq:hooke)) as a function of $x$. 

```{r, SpringSystem, fig.cap="Three springs with equilibrium ($F=0$) positions of $x=0$. Spring 1 is in compression, with restorative force acting in the $+x$ direction. Spring 2 is at equilibrium. Spring 3 is in tension, with restorative force acting in the $-x$ direction.", echo=FALSE}
knitr::include_graphics("figures/SpringSystem.svg")
```
...
Based on Eqs. \@ref(eq:hooke) and \@ref(eq:pe), why is the general shape of energy curve parabolic? Do some math to show us why.
...
You can confirm this on the graph in Fig. \@ref(fig:SpringPotential) by setting $k=1\,\text{N-m}$ and finding the value of $U$ at the values of $x$.
...
In this [_NetLogo_ model](https://ct-stem.s3.amazonaws.com/uploads/jzkelter/2021/08/18/draw%20potentials_2021-08-18-19-15-15-372549.html?speed=0.8), you will draw the interatomic potential function and see how the atoms react. The video demo embedded in Fig. \@ref(fig:KelterLecture) below shows how to use the model and demonstrates how you can draw an interatomic potential function similar to the potential energy function for an ideal spring that you reviewed above. For simplicity, the left atom is held fixed and only the right atom moves. You can think of this as looking at the word from the perspective (reference frame) of the left atom. From an outside perspective, both atoms would be moving. The force the right atom feels will be proportional to the slope of the interatomic potential function at its current location.

```{r, KelterLecture, fig.cap="How this _Netlogo_ model works.", echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/GJAeHGXRkis",height="400px")
```

Your job is to draw an interatomic potential function to model the way you think atoms behave, using the embedded model in Fig. \@ref(fig:SketchModel). We have already stated in the atomic hypothesis that atoms repel each other when squeezed together and attract each other when they are further apart. We can also deduce from observation that atoms must not attract each other very much when they are _very_ far apart, because otherwise everything would quickly condense into one giant blob. 

```{r, SketchModel, fig.cap="How this _Netlogo_ model works.", echo=FALSE}
knitr::include_url("https://ct-stem.s3.amazonaws.com/uploads/jzkelter/2021/08/18/draw%20potentials_2021-08-18-19-15-15-372549.html?speed=0.8",height="400px")
```
...
First, construct a spring potential like that in Fig. \@ref(fig:SpringPotential). Make some observations about how the atom behaves at different positions. Which features does the spring model have that fits atomic hypothesis? What is it missing?
...
The _NetLogo_ model in Fig. \@ref(fig:LJMD) is similar to the one you used earlier, but now, instead of you drawing the interatomic potential, it is modeled by the Lennard-Jones potential. You will see the two atoms with the LJ potential drawn between them from the perspective of the red atom. 
...
```{r, LJMD, fig.cap="A Lennard Jones Molecular Dynamics model.", echo=FALSE}
knitr::include_url("https://ct-stem.s3.amazonaws.com/uploads/jzkelter/2021/04/12/lj%20potential_2021-04-12-11-34-17-571484.html",height="400px")
```
